...
- `compliance/LGPDConsent.tsx` — LGPD consent UX
...
psycopg2-binary   >=2.9.9    # sync — use asyncpg for async contexts
sqlalchemy        >=2.0.0
python-dotenv     >=1.0.0
PyYAML            >=6.0.3
click             >=8.1.0
...
1. **securellm-bridge merge (OPEN):** Erroneous merge with cerebro. All `securellm-bridge` client code should be treated as potentially broken until surgical rollback is confirmed. Validate interface contracts before any calls.
...
4. **eBPF in Nix sandbox** — BPF syscalls blocked by `nix build`. eBPF code must be excluded from `nix build` checks and tested outside the sandbox.
...
7. **PostgreSQL port** — exposed on **5433** (not 5432). All connection strings must use port 5433.
...
uv run pytest -m integration         # include integration
