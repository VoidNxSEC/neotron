...
def main():
    ...
    state = OptimizationState(
        current_strategy=SearchStrategy.SEMANTIC, trials_completed=0, best_accuracy=0.0
    )
    ...
    with mlflow.start_run(run_name="semantic-search-demo") if mlflow else (lambda: None)():
        ...
        # Request suggestions
        configs = optimizer.suggest_configs(num=3, state=state, experiment_id=experiment_name)
